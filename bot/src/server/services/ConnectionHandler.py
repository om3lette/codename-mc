...
class ConnectionHandler:
    ...
    _cache_expire_time: dict[str, timedelta] = {
        ...
        "get_server_status": timedelta(seconds=5) if DEV_MODE else timedelta(minutes=1),
        "query_server": timedelta(seconds=5) if DEV_MODE else timedelta(minutes=1)
    }
    ...
    def __get_cache(self, function_name: str, user_id) -> tuple[Optional[RequestCacheSchema], bool]:
        ...
        if not has_expired and (user_id not in ADMIN_IDS or DEV_MODE):
