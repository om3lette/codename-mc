...
from python.bots.telegram.src.buttons.schemas import PaginatorConfig
...
class PaginatorBuilder:
	...
	def build(
		self, data: list[any], page_index: int, force_render: bool = False
	) -> [InlineKeyboardMarkup, PaginatorExitStatus]:
		...
		rows_to_add: int = math.ceil(
			len(current_page_items) / self.config.items_per_row
		)

		page_index_human: int = min(page_index + 1, total_pages)
		logging.debug(
			f"Markup update. Current page: {page_index_human}/{total_pages}. Elements to display: {len(current_page_items)}. Added rows: {rows_to_add}"
		)
		for i in range(rows_to_add):
			start_index: int = self.config.items_per_row * i
			buttons: list[InlineKeyboardButton] = [
				InlineKeyboardButton(
					text=self.config.item_to_text(current_page_items[j], j),
					callback_data="skip",
				)
				for j in range(
					start_index,
					min(
						start_index + self.config.items_per_row, len(current_page_items)
					),
				)
			]
		...
		nav_buttons_row.append(
			InlineKeyboardButton(
				text="⬅️",
				callback_data=callbacks.PaginationNav(
					action="prev", page=prev_page_index
				).pack(),
			)
		)
		nav_buttons_row.append(
			InlineKeyboardButton(
				text=f"{page_index_human}/{total_pages}", callback_data="skip"
			)
		)
		# Go to the next page
		...
		nav_buttons_row.append(
			InlineKeyboardButton(
				text="➡️",
				callback_data=callbacks.PaginationNav(
					action="next", page=next_page_index
				).pack(),
			)
		)
