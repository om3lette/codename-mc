...
class ConnectionHandler:
    ...
    _cache_expire_time: dict[str, timedelta] = {
        ...
        "get_server_status": timedelta(seconds=5) if config.dev_mode else timedelta(minutes=1),
        "query_server": timedelta(seconds=5) if config.dev_mode else timedelta(minutes=1)
    }
    ...
    async def __is_connected(self, error_callback: Callable = lambda *args: None) -> bool:
        ...
        while attempt_number <= config.connection.max_retries_before_hangup:
            try:
                ...
                await error_callback(attempt_number, attempt_number == config.connection.max_retries_before_hangup)
    ...
    def __get_cache(self, function_name: str, user_id) -> tuple[Optional[RequestCacheSchema], bool]:
        ...
        if expire_time is None:
            logging.warning(f"No cache expire time found for '{function_name}', cache is not used")
    ...
    async def execute(self, request_message: Message, func: Callable) -> tuple[bool, any]:
        cached_data, is_cache_valid = self.__get_cache(func.__name__, request_message.from_user.id)
        if is_cache_valid:
            return True, cached_data
        if not await self.__is_connected(lambda x, y: default_ping_error_callback(request_message, x, y)):
            return False, None
        response: any = await func()
        self._cache[func.__name__] = RequestCacheSchema(data=response, timestamp=datetime.now())
